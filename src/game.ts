import type { PositionDTO, BoardDTO, GameStateDTO, ShotDTO } from "./types";

// Figur Größen
const PIECE_SIZES = [2, 3, 4, 5] as const;
type PieceSize = typeof PIECE_SIZES[number];
// Figur Rotation
type PieceRotation = 'S' | 'E'
...
class Position {
    ...
    IsTouching(other: Position): boolean {
...
class Piece {
    // Board Positions covered by Piece
    public piecePositions: Position[]
    // Number of Hits received
    private hits: number

    constructor(
        public readonly size: PieceSize,
        public readonly rotation: PieceRotation,
        col: number,
        row: number
    ) {
        this.piecePositions = []
    ...
    CheckOverlap(position: Position): boolean {
    ...
    CheckHit(attackPosition: Position): boolean {
...
class Board {
    ...
    IsPositionShip(position: Position) {
        for (let piece of this.pieces) {
            if (piece.CheckOverlap(position)) return true
        }
        return false
    }
    ...
    private isPositionInsideBoard(position: Position): boolean {
    ...
    IsPiecePlacementValid(newPiece: Piece): boolean {
        // Check if Piece is inside Board
        for (let position of newPiece.piecePositions) {
            if (this.isPositionInsideBoard(position) == false) return false
        }
    ...
    TryPlacePiece(newPiece: Piece): boolean {
        // Check if Piece of this size can still be added
        if (this.piecesLeft[newPiece.size] < 1) return false

        // Fail safe, should never happen
        if (!this.IsPiecePlacementValid(newPiece)) return false
    ...
    IsShotValid(newShotPosition: Position): boolean {
        // Is shot inside board?
        if (this.isPositionInsideBoard(newShotPosition) == false) return false

        // Has shot already been fired?
        for (let shot of this.shots) {
            if (shot.position.Equal(newShotPosition)) return false
        }
    ...
    Shoot(shotPosition: Position): boolean {
        // Fail safe, should never happen
        if (!this.IsShotValid(shotPosition)) return false;
...
export class Game {
    ...
    private getRandomPiece(): Piece {
        let size: PieceSize = PIECE_SIZES[RandomIntInRange(0, 3)];
        let rotation: PieceRotation = RandomBool() ? 'E' : 'S';
        let col: number = RandomIntInRange(0, this.boardSize - 1);
        let row: number = RandomIntInRange(0, this.boardSize - 1);
    ...
    private isShotValid(board: Board, x: number, y: number): boolean {
        return board.IsShotValid(new Position(x, y))
    }

    private shoot(board: Board, x: number, y: number): boolean {
    ...
    private updateGameState(boardToCheck: Board) {
    ...
    HandlePlayerTurn(positionX: number, positionY: number): boolean {
